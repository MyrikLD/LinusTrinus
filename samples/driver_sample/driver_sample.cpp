...
public:
        CSampleDeviceDriver(  )
        {
                ...
                DriverLog( "driver_null: Using settings values\n" );
                ...
                DriverLog( "driver_null: RUN\n" );
                struct sockaddr_in local;
                DriverLog( "driver_null: 1\n" );
                memset((char *) &local, 0, sizeof(local));
                DriverLog( "driver_null: 2\n" );
                local.sin_family = AF_INET;
                DriverLog( "driver_null: 3\n" );
                local.sin_port = htons(4242);
                DriverLog( "driver_null: 4\n" );
                local.sin_addr.s_addr = htonl(INADDR_ANY);
                DriverLog( "driver_null: 5\n" );
                int iResult;
                DriverLog( "driver_null: 6\n" );
                socketS = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
                DriverLog( "driver_null: 7\n" );
                SetSocketBlockingEnabled(socketS, true);
                DriverLog( "driver_null: Socket created\n" );
                iResult = bind(socketS, (struct sockaddr*)&local, sizeof(local));
                if (iResult<0){
                    DriverLog( "driver_null: Socket activation error: %d\n", iResult );
                    SocketActivated = false;
                } else {
                    DriverLog( "driver_null: Socket activated\n" );
